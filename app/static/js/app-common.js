    ...
    window.addEventListener('load', () => {
      ...
      idle(() => {
        ...
        lazyMount('keywordTrends',     loadKeywordTrends);     // <section id="keywordTrends">
        lazyMount('extremeBubbles',    loadExtremeBubbles);    // <section id="extremeBubbles">
        lazyMount('weeklyChronicle',   loadWeeklyChronicle);   // <section id="weeklyChronicle">
      });
    });
...
document.getElementById('countryPanel').addEventListener('change', (e) => {
  ...
  function getFiltersFromURL() {
    ...
    const countries = new URLSearchParams(location.search).getAll('country');
    ACTIVE_COUNTRIES.clear();
    for (const c of countries) if (c && c.trim()) ACTIVE_COUNTRIES.add(c.trim().toUpperCase());
    ...
    if (from && to) {
      ...
      // Mehrere ?source=… auslesen
      const p2 = new URLSearchParams(location.search);
      const sources = p2.getAll('source');
      ACTIVE_SOURCES.clear();
      for (const s of sources) if (s && s.trim()) ACTIVE_SOURCES.add(s.trim());
      FILTERS.source = sources.length ? sources[0] : null; // Mirror für Altcode

      // Abwärtskompatibilität: ersten Eintrag auch in FILTERS.source spiegeln
      FILTERS.source = sources.length ? sources[0] : null;
    }
  }
  ...
  function updateURLFromFilters(replace=false) {
    ...
    if (ACTIVE_SOURCES.size > 0) {
      for (const s of ACTIVE_SOURCES) p.append('source', s);
    }
    if (ACTIVE_COUNTRIES.size > 0) {
      for (const c of ACTIVE_COUNTRIES) p.append('country', c);
    }
